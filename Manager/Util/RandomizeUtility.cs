...
public static class RandomizeUtility
{
    ...
    public static int TryGetRandomPlayerIndexByWeight(List<float> weights, List<int> targetRange) // 가중치 리스트를 받아온 후, 가중치에 기반한 확률대로 playerIndex 뽑기
    {
        float total = 0f;
        int playerIndex = -1;
        ...
        foreach (var weight in weights)
        {
            ...
            if (rand <= 0f) // 음수가 될 경우
            {
                playerIndex = targetRange[i]; // playerIndex 뽑음. 몇 번째 가중치인지 대입(playerIndex). 확률이 높으면 -되는 숫자도 커지니, 뽑힐 확률이 높아짐.
                break;
            }
            i++;
        }
